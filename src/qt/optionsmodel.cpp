...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSKWAmount"))
        settings.setValue("nAnonymizeSKWAmount", 1000);

    nAnonymizeSKWAmount = settings.value("nAnonymizeSKWAmount").toLongLong();
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in stakework.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSKWAmount:
            return QVariant(nAnonymizeSKWAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSKWAmount:
            nAnonymizeSKWAmount = value.toInt();
            settings.setValue("nAnonymizeSKWAmount", nAnonymizeSKWAmount);
            emit anonymizeSKWAmountChanged(nAnonymizeSKWAmount);
            break;
        case CoinControlFeatures:
